...
int main(int argc, char **argv) {
    ...
    const double wLen = std::ceil(anoG > catG ? 6.0 / anoG : 6.0 / catG);

    // DriftLineRKF maximum step size
    const double stepRKF = 0.001;
    ...
    cmp.SetGeometry(&geo);
    cmp.SetTargetElementSize(0.01);
    cmp.SetMinMaxNumberOfElements(1, 3); // fine-tune element size and number later
    cmp.UseLUInversion();
    ...
    std::cout   << "DriftLineRKF" << stepRKF << std::endl
                << "|   Integration accuracy: " << epsRKF << std::endl
                << "|   Max step size: " << epsRKF << std::endl;
